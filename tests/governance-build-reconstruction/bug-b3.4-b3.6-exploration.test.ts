 ...
 * - B3.5: safeCmd is NOT imported (only readWsFileAsync and writeWsFileAsync are imported)
...
describe("Bug B3.4-B3.6 Exploration: Unused Variables and Imports", () => {
  ...
  describe("Bug B3.5: safeCmd import in src/tools/tool-registry.ts - ALREADY FIXED", () => {
    ...
    it("should confirm safeCmd is NOT imported from utils (BUG FIXED)", () => {
      ...
      const importMatch = content.match(
        /import\s+\{([^}]+)\}\s+from\s+["']\.\.\/utils\/utils\.js["']/,
      );
    ...
    it("should verify only readWsFileAsync and writeWsFileAsync are imported", () => {
      // Find import statement from utils
      const importMatch = content.match(
        /import\s+\{([^}]+)\}\s+from\s+["']\.\.\/utils\/utils\.js["']/,
      );
      ...
      if (importMatch) {
        ...
        // Verify correct imports using word boundaries to avoid substring match issues
        expect(/\breadWsFileAsync\b/.test(imports)).toBe(true);
        expect(/\bwriteWsFileAsync\b/.test(imports)).toBe(true);
        expect(/\bsafeCmd\b/.test(imports)).toBe(false);
      }
    });
  ...
  describe("Bug B3.6: dirPath parameter in list_dir handler - ALREADY FIXED", () => {
    ...
    it("should confirm list_dir handler declares path parameter with underscore prefix (BUG FIXED)", () => {
      ...
      const listDirMatch = content.match(
        /list_dir:\s*async\s*\(\s*\{\s*path:\s*(\w+)\s*\}\s*\)/,
      );
    ...
    it("should verify list_dir returns hardcoded value (as expected for stub)", () => {
      ...
      if (listDirMatch) {
        ...
        const hasHardcodedReturn = /return\s*\{\s*files:\s*\[/.test(
          handlerBody,
        );
  ...
  describe("Summary: All three bugs are ALREADY FIXED", () => {
    it("should confirm all three bugs have been fixed", () => {
      ...
      const isSafeStdoutFixed =
        safeStdoutMatches && safeStdoutMatches.length >= 2;
      ...
      const listDirMatch = toolRegistryContent.match(
        /list_dir:\s*async\s*\(\s*\{\s*path:\s*(\w+)\s*\}\s*\)/,
      );
      ...
      expect(isSafeCmdFixed, "Bug B3.5 is FIXED: safeCmd is NOT imported").toBe(
        true,
      );
      ...
      // Overall summary
      const allBugsFixed =
        isSafeStdoutFixed && isSafeCmdFixed && isDirPathFixed;
