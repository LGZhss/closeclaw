  ...
  describe("Bug B3.5: safeCmd import in src/tools/tool-registry.ts - ALREADY FIXED", () => {
    ...
    it("should verify only readWsFileAsync and writeWsFileAsync are imported", () => {
      ...
      if (importMatch) {
        ...
        // Split and trim to avoid substring matches like readWsFile matching readWsFileAsync
        const importList = imports.split(",").map((s) => s.trim());

        // Verify correct imports
        expect(importList).toContain("readWsFileAsync");
        expect(importList).toContain("writeWsFileAsync");
        expect(importList).not.toContain("safeCmd");
      }
    });
