...
describe("fs-cleanup", () => {
  ...
  it("should delete temp files older than 1 hour", async () => {
    ...
    expect(fsPromises.stat).toHaveBeenCalledWith(path.join(MOCK_TMP_DIR, "temp_1.ts"));
    expect(fsPromises.stat).toHaveBeenCalledWith(path.join(MOCK_TMP_DIR, "temp_2.js"));
    expect(fsPromises.stat).not.toHaveBeenCalledWith(path.join(MOCK_TMP_DIR, "not_temp.ts"));
    expect(fsPromises.stat).not.toHaveBeenCalledWith(path.join(MOCK_TMP_DIR, "temp_3.txt"));
