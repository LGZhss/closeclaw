...
export const createToolRegistry = (
  ...
  return {
    ...
    read_file: async ({ path: filePath }) => {
      // What: Replace synchronous readWsFile with asynchronous readWsFileAsync.
      // Why: Synchronous file I/O blocks the Node.js event loop, preventing other concurrent LLM requests or tasks from being processed.
      // Impact: Improves overall application throughput and responsiveness under concurrent load by allowing the event loop to continue while waiting for file I/O.
      return { content: await readWsFileAsync(workspaceDir, filePath) };
    },

    /** 写入文件处理器 */
    write_file: async ({ path: filePath, content }) => {
      // What: Replace synchronous writeWsFile with asynchronous writeWsFileAsync.
      // Why: Synchronous file I/O blocks the Node.js event loop, which degrades performance during concurrent LLM operations.
      // Impact: Prevents event loop blocking during file writes, improving concurrency and reducing latency for parallel tool executions.
