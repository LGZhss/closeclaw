...
export const createToolRegistry = (
  ...
  return {
    ...
    read_file: async ({ path: filePath }) => {
      ...
      // Why: Synchronous file I/O blocks the Node.js event loop, preventing the processing of concurrent requests or other I/O events.
      // Impact: Significantly improves concurrent throughput under load by avoiding main thread blocking.
    ...
    write_file: async ({ path: filePath, content }) => {
      ...
      // Why: Synchronous file operations block the Node.js event loop, stalling concurrent executions.
      // Impact: Improves system responsiveness and throughput by keeping the event loop unblocked during disk I/O.
