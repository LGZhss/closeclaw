...
export const createToolRegistry = (
  ...
  return {
    ...
    read_file: async ({ path: filePath }) => {
      // What: Replaced synchronous readWsFile with asynchronous readWsFileAsync and awaited the result.
      // Why: Synchronous file operations block the Node.js event loop. Using the asynchronous version allows other concurrent requests to be processed, improving concurrency.
      // Impact: Increases concurrent request throughput by preventing event loop blocks during file reads.
      return { content: await readWsFileAsync(workspaceDir, filePath) };
    },

    /** 写入文件处理器 */
    write_file: async ({ path: filePath, content }) => {
      // What: Replaced synchronous writeWsFile with asynchronous writeWsFileAsync and awaited the result.
      // Why: Synchronous file writes block the Node.js event loop. Using the asynchronous version improves performance under concurrent workloads.
      // Impact: Prevents event loop blocking during file writes, improving application responsiveness.
