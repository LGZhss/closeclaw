...
export const createToolRegistry = (
  ...
  return {
    ...
    read_file: async ({ path: filePath }) => {
      ...
      // Why: Synchronous I/O blocks the Node.js event loop during concurrent LLM requests, degrading overall application throughput.
      // Impact: Eliminates event loop blocking during file reads, improving concurrency and responsiveness under load.
    ...
    write_file: async ({ path: filePath, content }) => {
      ...
      // Why: Synchronous I/O blocks the Node.js event loop during concurrent LLM requests, degrading overall application throughput.
      // Impact: Eliminates event loop blocking during file writes, improving concurrency and responsiveness under load.
