...
export const createToolRegistry = (
  ...
  return {
    ...
    read_file: async ({ path: filePath }) => {
      ...
      // Why: Synchronous file reads block the Node.js event loop, degrading performance during concurrent LLM tool calls.
      // Impact: Improves concurrency and reduces event loop lag when multiple file reads occur simultaneously.
    ...
    write_file: async ({ path: filePath, content }) => {
      ...
      // Why: Synchronous file writes block the Node.js event loop, delaying other concurrent operations in the server.
      // Impact: Eliminates blocking I/O during tool execution, allowing the server to handle more concurrent agent actions efficiently.
