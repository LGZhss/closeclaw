...
export const createToolRegistry = (
  ...
  return {
    ...
    read_file: async ({ path: filePath }) => {
      // What: Replace synchronous readWsFile with readWsFileAsync.
      // Why: Synchronous file operations block the Node.js event loop, degrading performance during concurrent LLM requests.
      // Impact: Allows the event loop to continue processing other concurrent requests while waiting for file I/O, improving throughput.
      return { content: await readWsFileAsync(workspaceDir, filePath) };
    },

    /** 写入文件处理器 */
    write_file: async ({ path: filePath, content }) => {
      // What: Replace synchronous writeWsFile with writeWsFileAsync.
      // Why: Synchronous file operations block the Node.js event loop, degrading performance during concurrent LLM requests.
      // Impact: Allows the event loop to continue processing other concurrent requests while waiting for file I/O, improving throughput.
