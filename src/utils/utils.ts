...
export function resolveSafePath(userPath: string): string {
  try {
    ...
    const safeWorkspacePath = realWorkspace.endsWith(path.sep)
      ? realWorkspace
      : realWorkspace + path.sep;
    if (
      realTarget !== realWorkspace &&
      !realTarget.startsWith(safeWorkspacePath)
    ) {
    ...
    const resolvedPath = path.resolve(WORKSPACE, userPath);
    const fallbackWorkspacePath = WORKSPACE.endsWith(path.sep)
      ? WORKSPACE
      : WORKSPACE + path.sep;
    if (
      resolvedPath !== WORKSPACE &&
      !resolvedPath.startsWith(fallbackWorkspacePath)
    ) {
      throw new Error(`Access denied: path is outside workspace (${userPath})`);
    }
    return resolvedPath;
